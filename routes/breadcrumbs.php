...
// Dashboard > Task
Breadcrumbs::for('task', function (BreadcrumbTrail $trail, $entity) {
    $trail->parent('phase', $entity);
    $trail->push(trans('admin::app.task.index.title'), route('admin.tasks.index', ['project_id' => $entity['project_id'] ?? '', 'phase_id' => $entity['phase_id'] ?? '']));
});
...
// Dashboard > Phase
Breadcrumbs::for('phase', function (BreadcrumbTrail $trail, $entity) {
    $trail->parent('project');
    $trail->push(trans('admin::app.phase.index.title'), route('admin.phases.index', $entity['project_id'] ?? ''));
});